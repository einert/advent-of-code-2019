    ...
    ([program input-num]
     (loop [memory program
       ...
       (let [op (get memory i)
         ...
         (case (mod op 100)
           1 (recur (assoc memory target (+ a b)) (+ i 4) result) ; add
           2 (recur (assoc memory target (* a b)) (+ i 4) result) ; mult
           3 (recur (assoc memory (get memory (inc i)) input-num) (+ i 2) result) ; get input
           4 (recur memory (+ i 2) (conj result a)) ; write output
           5 (recur memory (if (not= 0 a) b (+ i 3)) result) ; jump-if-true
           6 (recur memory (if (= 0 a) b (+ i 3)) result) ; jump-if-false
           7 (recur (assoc memory target (if (< a b) 1 0)) (+ i 4) result) ; less-than
           8 (recur (assoc memory target (if (= a b) 1 0)) (+ i 4) result) ; equals
